...
export function createTask(title) {
  return dispatch => {
    taskList.push({basket: false, title, qty: 1, freezer: true, drawer: 0})
...
export function undeleteTask() {
  return (dispatch, getState) => {
    ...
    if (task) {
      taskList.set(task.key, {basket: task.basket, title: task.title, freezer: task.freezer, drawer: task.drawer, qty: task.qty})
